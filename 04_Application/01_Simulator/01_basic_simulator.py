...
def control_device():
    global g_Radiator, g_Balcony_Windows, g_Door, g_Gas_Value
    ...
    elif menu_num == 2: g_Gas_Value = not g_Gas_Value
...
def smart_mode():
    ...
    menu_num = int(input('메뉴를 성택하세요: '))

    if menu_num == 1:
        print("현재 인공지능 모드: ", end='')
        if g_AI_Mode ==True:    print("작동 완료!")
        else:           print("중지")
    elif menu_num == 2:
...
print("<스마트 홈네트워크 시물레이터 프로그램 ver 1.0>")
print("                                         -손병찬-")
while True:
    print_main_menu()
