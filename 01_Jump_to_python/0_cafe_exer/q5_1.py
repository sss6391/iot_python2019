...
client = 0
free_ticket = [5, 3]
park_fare = {'유아':0, '어린이':2000, '청소년':3000, '성인':5000, '노인':0}
...
while True:
    ...
    if pay_type == 1:
        ...
        if input_money == park_fare[ident]:
            ...
            print("감사합니다. 티켓을 발행하고 거스름돈 %d원을 반환합니다." %(input_money-park_fare[ident]))
            client = client + 1
        else:
            print("%d원이 모자랍니다. 입력하신 %d원을 반환합니다." % (((input_money-park_fare[ident])*-1), input_money))
