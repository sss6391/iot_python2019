...
def mic_recoding(level):
    r = sr.Recognizer()
    mic = sr.Microphone()
    voice = ''

    if(level == 1):
        print('\n"'+g_input_command+'"이라고 말하세요!')
    elif(level == 2):
        print("\n명령어를 말하세요!")
    ...
    try:
        ...
        print("인식: " + r.recognize_google(audio, language="ko-KO"))
        voice = str(r.recognize_google(audio, language="ko-KO"))
    except sr.UnknownValueError:
        print("녹음된 음성에 문제가 있습니다.")
    except sr.RequestError as e:
        print("Cannot obtain results; {0}".format(e))
    except sr.WaitTimeoutError:
        print("녹음 시간이 초과되었습니다.")
...
def voice_mode():
    count = 0
    while True:
        input_state = mic_recoding(1)
        if input_state == "입력":
            command = mic_recoding(2)
            if command == '추천':
                print("추천 명령 인식")
                temperature()
                check(temperature_list)
...
def main_menu():
    while True:
        ...
        if(menu_num == 1):
            ...
            ai_scheduler = threading.Thread(target=voice_mode)
